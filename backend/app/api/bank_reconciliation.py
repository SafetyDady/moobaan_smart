...
# ===== TEMPORARY: Timezone Migration Audit (remove immediately after check) =====

@router.get("/audit/tz-migration")
async def audit_tz_migration(
    current_user: User = Depends(require_role(["super_admin"])),
    db: Session = Depends(get_db),
):
    """One-time audit: check if -7h shift affected any posted ledger entries."""
    from sqlalchemy import text
    
    result = {}

    # 1. Are there ANY income_transactions (posted ledger)?
    ledger_rows = db.execute(text("""
        SELECT it.id, it.house_id, it.payin_id, it.amount, it.received_at, 
               it.status, it.created_at,
               pr.transfer_date as payin_transfer_date,
               pr.status as payin_status
        FROM income_transactions it
        LEFT JOIN payin_reports pr ON it.payin_id = pr.id
        ORDER BY it.id
    """)).fetchall()
    
    result["total_ledger_entries"] = len(ledger_rows)
    ...
    for row in ledger_rows:
        entry = {
            ...
            "payin_transfer_date": str(row.payin_transfer_date) if row.payin_transfer_date else None,
            "payin_status": str(row.payin_status) if row.payin_status else None,
        }
        # Check if received_at and payin_transfer_date are in different months
        if row.received_at and row.payin_transfer_date:
            r_month = row.received_at.month
            p_month = row.payin_transfer_date.month
            entry["month_mismatch"] = r_month != p_month
        result["ledger_entries"].append(entry)

    # 2. Invoice payments linked to these ledger entries
    inv_pay_rows = db.execute(text("""
        SELECT ip.id, ip.income_transaction_id, ip.invoice_id, ip.amount, ip.status,
               inv.cycle_year, inv.cycle_month, inv.status as inv_status
        FROM invoice_payments ip
        JOIN invoices inv ON ip.invoice_id = inv.id
        ORDER BY ip.id
    """)).fetchall()
    
    result["total_invoice_payments"] = len(inv_pay_rows)
    result["invoice_payments"] = []
    for row in inv_pay_rows:
        result["invoice_payments"].append({
            "id": row.id,
            "income_txn_id": row.income_transaction_id,
            "invoice_id": row.invoice_id,
            "amount": float(row.amount),
            "payment_status": str(row.status),
            "invoice_cycle": f"{row.cycle_year}-{row.cycle_month:02d}",
    ...
    has_month_mismatch = any(e.get("month_mismatch") for e in result["ledger_entries"])
    
    result["verdict"] = {
        "has_posted_ledger": has_posted,
        "has_month_mismatch_after_shift": has_month_mismatch,
        "has_invoice_payments": len(inv_pay_rows) > 0,
        "safe": not has_month_mismatch,
    }
